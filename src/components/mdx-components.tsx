...
export function Hero({
  ...
  return (
    <section className="flex flex-col items-center text-center px-6 py-24 gap-6">
      ...
      <p className="text-lg text-[#6B6B6B] dark:text-[#999999] max-w-xl">
        {subtitle}
      </p>
      <Link
        href={ctaHref}
      ...
      {note && (
        <p className="text-sm text-[#6B6B6B] dark:text-[#999999]">{note}</p>
      )}
    </section>
  );
}
    ...
    <section className="px-6 py-16 bg-white dark:bg-[#1C1C1C] w-full">
      <div className="max-w-3xl mx-auto flex flex-col gap-0">
        {items.map((item, i) => (
          <div
            key={item.title}
            className="flex gap-6 py-8 border-b border-[#E5E5E5] dark:border-[#2C2C2C] last:border-0"
          >
            <span className="text-sm font-medium text-[#6B6B6B] dark:text-[#999999] tabular-nums mt-0.5 min-w-[1.5rem]">
              {String(i + 1).padStart(2, "0")}
            </span>
            <div>
              <h3 className="font-medium text-[#111111] dark:text-[#F5F4EF] mb-1">
                {item.title}
              </h3>
  ...
  return (
    <section className="px-6 py-12 max-w-3xl mx-auto w-full">
      <div className="bg-[#111111] dark:bg-[#F5F4EF] rounded-2xl p-8 text-center">
        <p className="text-4xl font-medium text-white dark:text-[#111111] mb-2">
          Keep {keep}%
        </p>
        <p className="text-[#999999] dark:text-[#6B6B6B] text-sm">
          We take a 4.5% platform fee. Stripe fees apply separately. No monthly
          plans, no hidden cuts.
        </p>
      </div>
    </section>
  );
...
export function CTA({
  ...
  return (
    <section className="px-6 py-20 flex flex-col items-center gap-4 text-center">
      ...
      {sub && (
        <p className="text-sm text-[#6B6B6B] dark:text-[#999999]">{sub}</p>
      )}
    </section>
  );
}
      ...
      <div className="flex flex-col divide-y divide-[#E5E5E5] dark:divide-[#2C2C2C]">
        {items.map((item) => (
          <div key={item.q} className="py-6">
            <p className="font-medium text-[#111111] dark:text-[#F5F4EF] mb-2">
              {item.q}
            </p>
            <p className="text-sm text-[#6B6B6B] dark:text-[#999999]">
              {item.a}
...
export const mdxComponents: MDXComponents = {
  ...
  a: ({ href = "#", ...props }) => (
    <Link
      href={href}
      className="underline text-[#111111] dark:text-[#F5F4EF] hover:opacity-60"
      {...props}
    />
  ),
};
